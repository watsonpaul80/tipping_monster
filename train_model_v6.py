...
def preprocess(df):
    ...
    df["rpr"] = df["rpr"].astype(str).str.extract(r"(\d+)", expand=False).astype(float).fillna(-1)
    df["going"] = df["going"].astype(str).str.strip().astype("category").cat.codes
    df["class"] = df["class"].astype(str).str.extract(r"(\d+)", expand=False).astype(float).fillna(-1)
...
def train_model(df, feature_cols):
    ...
    if y.sum() == 0 or y.sum() == len(y):
        raise ValueError("Error: No class variance in target variable ('won')! Check your data.")
