...
MIN_VOLUME = 500  # Minimum matched volume threshold in £
...
def detect_steamers(merged_data):
    ...
    for runner in merged_data:
        ...
        if len(odds_prog) < 2 or None in odds_prog:
            continue

        volume = runner.get("volume", 0) or 0
        if volume < MIN_VOLUME:
            # skip low volume steamers
            continue
    ...
    logging.info(f"Detected {len(steamers)} steamers with ≥30% drop and volume ≥{MIN_VOLUME}")
    return steamers
